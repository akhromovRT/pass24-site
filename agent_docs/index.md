# Навигация по документам

Открывать только релевантные файлы — не читать всё подряд.

## Ключевые документы (v2.0)
- `agent_docs/blueprint.md` — **маркетинговый blueprint**: позиционирование, контент всех страниц, CTA, копирайтинг, SEO, дорожная карта. Читать при работе над контентом, текстами, структурой страниц.
- `agent_docs/technical-spec.md` — **техническое ТЗ**: стек WordPress, плагины, интеграция Битрикс24, аналитика, скоринг лидов, автопубликация, nurture-цепочки. Читать при разработке и настройке.

## Основные
- `agent_docs/architecture.md` — техстек, карта сайта, дизайн-система, хостинг. Читать при изменениях системы или настройке нового компонента.
- `agent_docs/implementation-plan.md` — детальный чек-лист по 6 фазам (Фаза 1: фундамент → Фаза 6: оптимизация). Читать при выполнении конкретных шагов.
- `agent_docs/adr.md` — 11 архитектурных решений: техстек (ADR-001–005), стратегия контента и конверсии (ADR-006–011).
- `agent_docs/development-history.md` — журнал итераций; смотреть последние 10 записей.

## Стратегия и контент
- `agent_docs/content-strategy.md` — SEO-кластеры, контент-план блога (10 статей), формулы CTA, 7 столпов позиционирования, конкурентные преимущества.
- `agent_docs/integrations.md` — автоматизация продаж: поток лидов WordPress → Битрикс24, модель скоринга, n8n автопубликация, JivoSite бот, nurture email-цепочка, многошаговая форма.

## Правила и гайды
- `agent_docs/guides/dod.md` — критерии завершённости задачи.
- `agent_docs/guides/environment-setup.md` — настройка .env, .cursorignore, .vscode.
- `agent_docs/guides/logging.md` — логирование скриптов и интеграций.
- `agent_docs/guides/archiving-and-temp.md` — архивация и временные файлы.

## Шаблоны
- `agent_docs/templates/architecture.md`
- `agent_docs/templates/adr.md`
- `agent_docs/templates/development-history.md`
